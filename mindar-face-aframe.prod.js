(() => {
  const { Controller: e, UI: t } = window.MINDAR.FACE,
    i = AFRAME.THREE;
  AFRAME.registerSystem("mindar-face-system", {
    ...
    setup: function ({
      ...
      (this.ui = new t({ uiLoading: e, uiScanning: i, uiError: s })),
        (this.filterMinCF = a),
        (this.filterBeta = n);
    },
    registerFaceMesh: function (e) {
      this.faceMeshEntities.push({ el: e });
    },
    registerAnchor: function (e, t) {
      this.anchorEntities.push({ el: e, anchorIndex: t });
    },
    start: function () {
      this.ui.showLoading(),
    ...
    _startVideo: function () {
      ...
      navigator.mediaDevices
        .getUserMedia({
          audio: !1,
          video: { facingMode: this.shouldFaceUser ? "face" : "environment" },
        })
        .then((e) => {
    ...
    _processVideo: function () {
      (this.controller.onUpdate = ({ hasFace: e, estimateResult: t }) => {
        if (
          (e && !this.lastHasFace && this.el.emit("targetFound"),
          !e && this.lastHasFace && this.el.emit("targetLost"),
          (this.lastHasFace = e),
          e)
        ) {
          const { faceMatrix: e } = t;
          for (let e = 0; e < this.anchorEntities.length; e++) {
            const t = this.controller.getLandmarkMatrix(
              this.anchorEntities[e].anchorIndex
            );
            this.anchorEntities[e].el.updateVisibility(!0),
              this.anchorEntities[e].el.updateMatrix(t);
          }
          for (let t = 0; t < this.faceMeshEntities.length; t++)
            this.faceMeshEntities[t].el.updateVisibility(!0),
              this.faceMeshEntities[t].el.updateMatrix(e);
        } else {
          for (let e = 0; e < this.anchorEntities.length; e++)
            this.anchorEntities[e].el.updateVisibility(!1);
          for (let e = 0; e < this.faceMeshEntities.length; e++)
            this.faceMeshEntities[e].el.updateVisibility(!1);
        }
      }),
    ...
    _setupAR: async function () {
      ...
      for (let e = 0; e < this.faceMeshEntities.length; e++)
        this.faceMeshEntities[e].el.addFaceMesh(
          this.controller.createThreeFaceGeometry(i)
        );
      this._resize(),
    ...
    _resize: function () {
      ...
      a > t.clientWidth / t.clientHeight
        ? ((s = t.clientHeight), (i = s * a))
        : ((i = t.clientWidth), (s = i / a)),
        (this.video.style.top = -(s - t.clientHeight) / 2 + "px"),
        (this.video.style.left = -(i - t.clientWidth) / 2 + "px"),
        (this.video.style.width = i + "px"),
        (this.video.style.height = s + "px");
      const n = t.getElementsByTagName("a-scene")[0];
      (n.style.top = this.video.style.top),
        (n.style.left = this.video.style.left),
        (n.style.width = this.video.style.width),
        (n.style.height = this.video.style.height);
    },
  }),
    AFRAME.registerComponent("mindar-face", {
      ...
      init: function () {
        ...
        if (this.data.faceOccluder) {
          ...
          e.setAttribute("mindar-face-default-face-occluder", !0),
    ...
    AFRAME.registerComponent("mindar-face-target", {
      dependencies: ["mindar-face-system"],
      schema: { anchorIndex: { type: "number" } },
      init: function () {
        this.el.sceneEl.systems["mindar-face-system"].registerAnchor(
          this,
          this.data.anchorIndex
        );
        ...
        this.el.object3D,
          this.el.addEventListener("model-loaded", () => {
            this.el.getObject3D("mesh").traverse((e) => {
              if (e.isMesh) {
                const t = new i.MeshStandardMaterial({ colorWrite: !1 });
                e.material = t;
              }
            });
          });
    ...
    AFRAME.registerComponent("mindar-face-default-face-occluder", {
      ...
      addFaceMesh(e) {
        const t = new i.MeshBasicMaterial({ colorWrite: !1 }),
          s = new i.Mesh(e, t);
